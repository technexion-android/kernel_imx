...
struct otp_flash *otp_flash_init_ar0521(struct device *dev)
{
	return NULL;
}

u16 otp_flash_get_checksum_ar0521(struct otp_flash *instance)
{
	return BOOTDATA_CHECKSUM;
}

size_t otp_flash_read_ar0521(struct otp_flash *instance, u8 *data, int addr, size_t len)
{
	size_t l;
...
size_t otp_flash_get_pll_length_ar0521(struct otp_flash *instance)
{
	return BOOTDATA_PLL_INIT_SIZE;
}

size_t otp_flash_get_pll_section_ar0521(struct otp_flash *instance, u8 *data)
{
	otp_flash_read_ar0521(instance, data, 0, BOOTDATA_PLL_INIT_SIZE);
	return BOOTDATA_PLL_INIT_SIZE;
}
#else
...
size_t otp_flash_get_pll_section_ar0521(struct otp_flash *instance, u8 *data)
{
	...
	if(temp[0] == 2) {
		...
		if (header->pll_bootdata_len != 0) {
			otp_flash_read_ar0521(instance, data, 0,
