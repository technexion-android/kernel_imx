				       ...
				       const struct i2c_client *client, u16 alias)
{
	...
	return max_des_ser_attach_client(priv, chan_id, client->addr, alias);
}

// static void max_des_ser_atr_detach_client(struct i2c_atr *atr, u32 chan_id, u16 addr)
...
static int max_des_i2c_atr_init(struct max_des_priv *priv)
{
	...
	for (i = 0; i < des->ops->num_links; i++) {
		...
		ret = i2c_atr_add_adapter(priv->atr, i, NULL, NULL);
		if (ret)
