...
struct resolution res_list_ar0234[] = {
...
static int ops_power_ar0234(struct v4l2_subdev *sub_dev, int on)
{
...
static int ops_init_ar0234(struct v4l2_subdev *sub_dev, u32 val)
{
	//struct sensor *instance = container_of(sub_dev, struct sensor, v4l2_subdev);
...
static int ops_load_fw_ar0234(struct v4l2_subdev *sub_dev)
{
	//struct sensor *instance = container_of(sub_dev, struct sensor, v4l2_subdev);
...
static int ops_reset_ar0234(struct v4l2_subdev *sub_dev, u32 val)
{
	//struct sensor *instance = container_of(sub_dev, struct sensor, v4l2_subdev);
...
static int ops_get_frame_interval_ar0234(struct v4l2_subdev *sub_dev,
...
static int ops_set_frame_interval_ar0234(struct v4l2_subdev *sub_dev,
				  struct v4l2_subdev_frame_interval *fi)
{
	dev_dbg(sub_dev->dev, "%s()\n", __func__);
...
static int ops_set_stream_ar0234(struct v4l2_subdev *sub_dev, int enable)
{
	struct sensor *instance = container_of(sub_dev, struct sensor, v4l2_subdev);
	int ret = 0;
	...
	if (instance->selected_mode >= ARRAY_SIZE(res_list_ar0234))
		return -EINVAL;
	...
	if (enable == 0) {
		...
		if (ret == 0) {
			...
			sensor_i2c_write_16b(instance->i2c_client, 0x4000,
					     res_list_ar0234[instance->selected_mode].width);
			//VIDEO_HEIGHT
			sensor_i2c_write_16b(instance->i2c_client, 0x4002,
					     res_list_ar0234[instance->selected_mode].height);
...
static int ops_enum_mbus_code_ar0234(struct v4l2_subdev *sub_dev,
...
static int ops_get_fmt_ar0234(struct v4l2_subdev *sub_dev,
		       struct v4l2_subdev_pad_config *cfg,
...
static int ops_set_fmt_ar0234(struct v4l2_subdev *sub_dev,
		       struct v4l2_subdev_pad_config *cfg,
		       struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *fmt;
	...
	for(i = 0 ; i < ARRAY_SIZE(res_list_ar0234) ; i++)
	{
		if (mbus_fmt->width == res_list_ar0234[i].width &&
				mbus_fmt->height == res_list_ar0234[i].height)
			break;
	}

	if (i >= ARRAY_SIZE(res_list_ar0234))
	{
		return -EINVAL;
	}
	...
	mbus_fmt->width = res_list_ar0234[i].width;
	mbus_fmt->height = res_list_ar0234[i].height;
...
static int ops_enum_frame_size_ar0234(struct v4l2_subdev *sub_dev,
			       struct v4l2_subdev_pad_config *cfg,
			       struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if ((fse->pad != 0) ||
	    (fse->index >= ARRAY_SIZE(res_list_ar0234)))
		return -EINVAL;

	fse->min_width = fse->max_width = res_list_ar0234[fse->index].width;
	fse->min_height = fse->max_height = res_list_ar0234[fse->index].height;

	return 0;
}

static int ops_enum_frame_interval_ar0234(struct v4l2_subdev *sub_dev,
				   struct v4l2_subdev_pad_config *cfg,
...
static int ops_media_link_setup_ar0234(struct media_entity *entity,
...
static const struct v4l2_subdev_core_ops sensor_v4l2_subdev_core_ops = {
	.s_power = ops_power_ar0234,
	.init = ops_init_ar0234,
	.load_fw = ops_load_fw_ar0234,
	.reset = ops_reset_ar0234,
};
static const struct v4l2_subdev_video_ops sensor_v4l2_subdev_video_ops = {
	.g_frame_interval = ops_get_frame_interval_ar0234,
	.s_frame_interval = ops_set_frame_interval_ar0234,
	.s_stream = ops_set_stream_ar0234,
};
static const struct v4l2_subdev_pad_ops sensor_v4l2_subdev_pad_ops = {
	.enum_mbus_code = ops_enum_mbus_code_ar0234,
	.get_fmt = ops_get_fmt_ar0234,
	.set_fmt = ops_set_fmt_ar0234,
	.enum_frame_size = ops_enum_frame_size_ar0234,
	.enum_frame_interval = ops_enum_frame_interval_ar0234,
};
...
static const struct media_entity_operations sensor_media_entity_ops = {
	.link_setup = ops_media_link_setup_ar0234,
};
...
static int sensor_load_bootdata(struct sensor *instance)
{
	...
	checksum = otp_flash_get_checksum_ar0234(instance->otp_flash_instance);

	//load pll
	bootdata_temp_area[0] = cpu_to_be16(BOOT_DATA_START_REG);
	pll_len = len = otp_flash_get_pll_section_ar0234(instance->otp_flash_instance,
	...
	len = otp_flash_read_ar0234(instance->otp_flash_instance,
			     (u8 *)(&bootdata_temp_area[1]),
...
static int sensor_probe_ar0234(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	instance->otp_flash_instance = otp_flash_init_ar0234(dev);
	...
	fmt = &instance->fmt;
	fmt->width = res_list_ar0234[0].width;
	fmt->height = res_list_ar0234[0].height;
...
static int sensor_remove_ar0234(struct i2c_client *client)
{
	return 0;
}
...
static struct i2c_driver sensor_i2c_driver = {
	...
	.probe = sensor_probe_ar0234,
	.remove = sensor_remove_ar0234,
	.id_table = sensor_id,
};
